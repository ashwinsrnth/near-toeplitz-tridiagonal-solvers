...
__global__ void sharedMemCyclicReduction( double *a_d,
    ...
    */
    ...
    d_l[li3d0+2*lix] = d_d[i3d0+2*lix];
    d_l[li3d0+2*lix+1] = d_d[i3d0+2*lix+1];
    __syncthreads();
        ...
        if (lix < nx/stride) {
            ...
            if (stride == nx) {
                if (lix == 0) {
                    ...
                    d_m = (d_l[li3d0+nx/2-1]*b_d[n] - \
                           c_d[m]*d_l[li3d0+nx-1])/ \
                        (b_first_d[m]*b_d[n] - c_d[m]*a_d[n]);

                    d_n = (b_first_d[m]*d_l[li3d0+nx-1] - \
                           d_l[li3d0+nx/2-1]*a_d[n])/ \
                        (b_first_d[m]*b_d[n] - c_d[m]*a_d[n]);
    ...
    for (int step=0; step<rint(log2((float) nx))-1; step++) {
        ...
        if (lix < nx/stride){
            ...
            if (stride == 2) {
                if (lix == 0) {
                    d_l[ix] = (d_l[ix] - c1*d_l[ix+1])/b1;
                }
                else {
                    d_l[ix] = (d_l[ix] - ai*d_l[ix-1] - ci*d_l[ix+1])/ \
                                bi;
                } 
            }

            else {
                idx = rint(log2((float) stride)) - 2;
                if (lix == 0) {
    ...
    d_d[i3d0+2*lix] = d_l[li3d0+2*lix];
    d_d[i3d0+2*lix+1] = d_l[li3d0+2*lix+1];
    __syncthreads();
}
